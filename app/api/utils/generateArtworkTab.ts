...
export const generateArtworkTab = (videos: VideoModel[]) => {
    ...
    videos.forEach(video => {
        if (video.textlessArtFilename) {
            let row = [video.guid, video.textlessArtFilename, "Textless", moment().format('YYYY-MM-DDTHH:mm:ss') + "Z"];
            content.push(row)
        }
        else {
        ...
        if ( video.verticalArtFilename){
            let row = [video.guid, video.verticalArtFilename, "Texted", moment().format('YYYY-MM-DDTHH:mm:ss') + "Z"];
            content.push(row)
        }
        else {
        ...
        if (video.artFilename) {
            let row = [video.guid, video.artFilename, "",  moment().format('YYYY-MM-DDTHH:mm:ss') + "Z"];
            content.push(row)
        }
        else {
